...
class GlobalExceptionHandler {
    ...
    fun handleInvalidAppleToken(ex: InvalidAppleTokenException): ResponseEntity<ApiError> =
        ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
            ApiError(status = 401, error = "Unauthorized", message = ex.message ?: "Invalid Apple identity token"),
        )

    @ExceptionHandler(InvalidSessionTokenException::class)
    fun handleInvalidSession(ex: InvalidSessionTokenException): ResponseEntity<ApiError> =
        ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
            ApiError(status = 401, error = "Unauthorized", message = ex.message ?: "Invalid session token"),
        )
